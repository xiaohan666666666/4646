...
if not WHITELIST[player.UserId] then
    ...
    UnauthTab:CreateLabel("请联系作者添加白名单后使用")
...
-- 玩家功能标签页
local FunctionTab = Window:CreateTab("拔出一把剑功能", 4483345998)
...
-- 新增【越狱】标签页（终点循环刷取功能）
local 越狱Tab = Window:CreateTab("越狱", 4483345998)

-- 越狱功能变量（隔离命名，避免冲突）
local 越狱_isRunning = false
local 越狱_character, 越狱_rootPart
local 越狱_finishPart -- 终点部件
local 越狱_cashObj -- 现金对象
local 越狱_loopCoroutine -- 循环协程

-- 1. 现金对象查找（越狱功能专用）
local function 越狱_findCash()
    ...
    for _, name in ipairs(cashNames) do
        ...
        if obj and obj:IsA("IntValue") then
            越狱_cashObj = obj
            Rayfield:Notify({Name = "越狱", Content = "找到现金："..obj:GetFullName(), Duration = 2})
            return
        end
    end
    Rayfield:Notify({Name = "越狱", Content = "未找到现金（不影响刷取）", Duration = 5})
end

-- 2. 终点搜索（越狱功能专用）
local function 越狱_findFinish()
    越狱_finishPart = nil
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj.Name:match(finishPattern) and obj:IsA("BasePart") then
            越狱_finishPart = obj
            Rayfield:Notify({Name = "越狱", Content = "找到终点："..obj.Name, Duration = 2})
            return
        end
    end
    Rayfield:Notify({Name = "越狱", Content = "未找到终点！按刷新重试", Duration = 5})
end

-- 3. 防落水传送（越狱功能专用）
local function 越狱_teleportTo(targetPart)
    local downRay = Ray.new(targetPart.Position, Vector3.new(0, -100, 0))
    local hit, position = Workspace:FindPartOnRay(downRay, targetPart)
    local safePosition = targetPart.Position + Vector3.new(0, 3, 0) -- 保底高度
    
    -- 检测到有效地面（非水域/透明）
    if hit and hit.Transparency < 1 and not hit:IsA("Terrain") then 
        safePosition = position + Vector3.new(0, 1, 0) -- 地面上方1格
    end
    
    越狱_rootPart.CFrame = CFrame.new(safePosition)
    Rayfield:Notify({Name = "越狱", Content = "已传送至"..targetPart.Name, Duration = 1})
end

-- 4. 终点循环逻辑（越狱功能专用）
local function 越狱_autoLoop()
    while 越狱_isRunning do
        -- 终点缺失时自动重找
        if not 越狱_finishPart then 
            越狱_findFinish()
            task.wait(3)
            if not 越狱_finishPart then continue end
        end
        
        -- 传送至终点并等待结算
        越狱_teleportTo(越狱_finishPart)
        task.wait(2)
        
        -- 现金反馈
        if 越狱_cashObj then
            Rayfield:Notify({Name = "越狱", Content = "现金："..越狱_cashObj.Value, Duration = 1})
        end
...
越狱Tab:CreateToggle({
    ...
    Callback = function(state)
        ...
        if state then
            -- 初始化角色
            越狱_character = player.Character or player.CharacterAdded:Wait()
            越狱_rootPart = 越狱_character:WaitForChild("HumanoidRootPart")
            ...
            -- 启动循环协程
            越狱_loopCoroutine = coroutine.create(越狱_autoLoop)
            coroutine.resume(越狱_loopCoroutine)
            Rayfield:Notify({Name = "越狱", Content = "终点循环已启动", Duration = 2})
        else
            -- 强制终止协程
            if 越狱_loopCoroutine then
                coroutine.close(越狱_loopCoroutine)
                越狱_loopCoroutine = nil
            end
            越狱_isRunning = false
            Rayfield:Notify({Name = "越狱", Content = "终点循环已停止", Duration = 2})
        end
    end
})
...
越狱Tab:CreateLabel("功能说明：持续传送至终点并触发结算，自动防落水")