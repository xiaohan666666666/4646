当地的本地白名单 = {
    ...
    [6039062907]= 真，6039062907] = 真实的,6039062907]= 真，6039062907] = 真实的,
    [8080735686]= 真，8080735686] = 真实的,8080735686]= 真，8080735686] = 真实的,
    [[5074612207]=真实，—新增白名单7079287947] = 真实的，——新增白名单[[5074612207]]=真实，—新增白名单7079287947] = 真实的, -- 新增白名单ID
    [3937432816]=真实—新增白名单3937432816] = 真实的  3937432816]=真实—新增白名单3937432816] = 真实的  -- 新增白名单ID
    -- 可继续添加白名单标识-- 可继续添加白名单标识
}

-- 加载猎户座（Orion）库
当地的本地Orion lib
当地的局部成功，err=pcall（功能（）功能（）
OrionLib = loadstring（game:HttpGet（"https://pastebin.com/raw/FUEx0f3G"））（）"https://pastebin.com/raw/FUEx0f3G"))()"https://pastebin.com/raw/FUEx0f3G"））（）"https://pastebin.com/raw/FUEx0f3G"))()
结束结束）
如果不成功或不OrionLib然后
警告（"猎户座（猎户座）库加载失败:"..（错误或"未知错误"））"猎户座（猎户座）库加载失败:"..（错误或 "未知错误"未知错误"))"猎户座（猎户座）库加载失败:"..（错误或"未知错误"））"猎户座（猎户座）库加载失败:"..（错误或 "未知错误"未知错误"))
    返回
结束

-- 服务与变量初始化
当地的当地的玩家=游戏：获得服务（"玩家"）"玩家")"玩家")
当地的本地玩家=玩家。
当地的“复制存储"）"复制存储")"复制存储"）"复制存储")

-- 白名单验证
如果不白名单【玩家.用户标识】然后
函数（）功能（）
游戏：获得服务（"启动界面"）：设置核心（"发送通知"，{"启动界面"）:设置核心（"发送通知", {"StarterGui"）:设置核心（"发送通知"， {"StarterGui"）:设置核心（"发送通知", {
标题="未授权访问","未授权访问","未授权访问","未授权访问",
文本 ="您的身份证不在白名单中","您的身份证不在白名单中","您的身份证不在白名单中","您的身份证不在白名单中",
持续时间=5，5,5，5,
图标=“rbxassetid://9108657181""rbxassetid://9108657181""rbxassetid://9108657181""rbxassetid://9108657181"
        })
    结束)
    当地的限制窗口=东方利布：制造窗口（{
名称="访问受限","访问受限","访问受限","访问受限","访问受限",
        Theme = "Dark",
